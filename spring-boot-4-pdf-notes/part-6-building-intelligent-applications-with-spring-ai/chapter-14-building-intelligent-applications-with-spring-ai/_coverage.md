...
노트 19개로 미검증 챕터 중 가장 컸고 Spring AI는 API 변동이 잦은 영역이라 대조했으나, 어긋난 곳을 찾지 못했다. LLM 주제에서 가장 흔한 오해 셋을 기준으로 확인했다.

| 흔한 오해 | 이 챕터의 서술 |
|---|---|
| **"모델이 도구를 실행한다"** | `04` §2.2가 툴 콜링을 *"model이 답을 만드는 대신 application의 **method 실행을 요청**하는 방식"*으로 정의한다. **요청과 실행의 주체를 옳게 분리**했고, "결과를 답에 반영하는 방식"까지 이어 붙였다 |
| **"컨텍스트 윈도는 입력 길이 제한이다"** | `01` §2.1이 *"**prompt와 생성될 응답이 같은 예산을 나눠 쓴다.** 대화 이력 20턴을 넣으면 그만큼 답이 짧아진다"*로 가장 놓치기 쉬운 지점을 짚는다 |
| **"temperature 0이면 결정적이다"** | `01` §2.1이 *"낮으면 같은 질문에 같은 답이 나오기 **쉽고**"*로 적절히 유보한다. 단정하지 않았다 |

추가로 확인한 것.

| 확인한 것 | 결과 |
|---|---|
| 임베딩 차원 불일치 | `05a` §2.6이 *"model의 출력 차원과 **반드시 같아야 한다**"*고 못박고, **증상의 시점**("애플리케이션이 뜨고 나서 첫 색인 때 터진다")과 **모델 교체 시 재색인 필요**까지 적는다 |
| 코사인 거리를 쓰는 이유 | "벡터의 크기가 아니라 **방향**만 비교한다. 문장 길이에 덜 휘둘린다" — 정확하다 |
