...
| `git diff --check` | PASS |
| `part-0-jpa-foundations/` | 읽기·수정 없음 |
...
- `part-0-jpa-foundations/`는 다른 세션이 작성 중이라 이 트랙에서 읽기·수정·링크 생성 대상이 아니다.
...
저장소 전체 스윕(part-0 제외, 코드펜스·인라인 코드스팬 제외, Obsidian의 오른쪽 부분경로 매칭 적용) 결과 **위키 링크 2,916개 중 미해결 0개**다. 저장소에 깨진 내부 링크가 하나도 남아 있지 않다.
