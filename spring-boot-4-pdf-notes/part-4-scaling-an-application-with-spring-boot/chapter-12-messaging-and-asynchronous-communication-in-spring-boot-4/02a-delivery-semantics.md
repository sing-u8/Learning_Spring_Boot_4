...
> **범위를 짚어야 §2.4의 결론이 성립한다.** Kafka가 제공하는 exactly-once는 **Kafka 안에서 읽고·처리하고·다시 Kafka에 쓰는 경로**에 한정된다. 소비한 오프셋 커밋과 새 메시지 발행을 **하나의 트랜잭션으로 묶는** 방식이라 그 범위 안에서는 성립한다.
>
> **그 범위를 벗어나는 순간 보장이 끝난다.** 메시지를 처리하며 이메일을 보내거나, 외부 REST API를 호출하거나, 트랜잭션에 참여하지 않는 저장소에 쓰면 — 그 부수 효과는 Kafka 트랜잭션 밖이다. 재시도가 일어나면 **이메일은 두 번 나간다.**
>
> 이것이 §2.4의 "at-least-once + 멱등 소비자"가 실무의 답인 진짜 이유다. 대부분의 비즈니스 애플리케이션은 **Kafka 밖으로 나가는 부수 효과**를 갖는데, exactly-once는 바로 그 부분을 덮어 주지 못한다. "복잡해서 안 쓴다"보다 **"내가 필요한 곳을 덮어 주지 않아서 안 쓴다"**가 더 정확하다.
...
> 네 문항을 스스로 답한 **뒤에** [[_02a-delivery-semantics]]에서 모범답안과 대조한다. 먼저 열면 이 문항들은 다시 인출 문제로 쓸 수 없다.
