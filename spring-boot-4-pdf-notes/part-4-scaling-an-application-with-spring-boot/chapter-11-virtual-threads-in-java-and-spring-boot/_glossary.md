...
가상 스레드가 캐리어 스레드에서 **내려오지 못하고 붙들리는** 상태. 이 구간에서는 블로킹이 일어나도 언마운트되지 않으므로 가상 스레드의 이점이 사라진다.

Oracle Java 25 문서의 규정 — *"가상 스레드는 캐리어에 핀닝돼 있을 때 블로킹 연산 중에 언마운트될 수 없다. 가상 스레드는 **`native` 메서드나 외부 함수(foreign function)를 실행할 때** 핀닝된다."* 그리고 *"핀닝이 애플리케이션을 틀리게 만들지는 않지만 확장성을 저해할 수 있다."*

**버전에 민감한 용어다.** JDK 21~23에서는 `synchronized` 블록·메서드 안의 블로킹도 핀닝을 일으켰고 "`ReentrantLock`을 쓰라"는 권고가 여기서 나왔다. **JDK 24의 JEP 491이 이를 해소해**, 이 책이 쓰는 Java 25에서는 `synchronized`가 핀닝 원인이 아니다. 옛 자료를 읽을 때 이 경계를 확인해야 한다.
