...
메커니즘 밀도가 높아 우선순위 상위로 대조했으나 어긋난 곳을 찾지 못했다. 오히려 **이 챕터가 이미 잘하고 있는 것**을 기록해 둔다 — 다른 챕터 검증의 기준이 된다.

| 확인한 것 | 결과 |
|---|---|
| "네이티브에서 리플렉션·프록시가 지원되지 않는다" | `02` §2.2가 **"이 서술은 거짓"**이라고 직접 바로잡고 *"사용의 제약이지 지원의 부재가 아니다"*로 구분한다. 리플렉션은 등록하면 되고, 프록시는 런타임 생성만 안 된다 |
| 책의 수치(0.1초) vs 실측(0.528초) | `03a` §5가 불일치를 짚고 **실측값으로 다시 계산**해 "1,000 인스턴스 약 8.8분"을 제시한다. *"논증의 방향은 맞고 숫자만 낙관적이다"*로 판정까지 남겼다 |
| 네 전략 비교의 수치 | `07b`가 *"정확한 수치는 애플리케이션·워크플로·컨테이너 구성·하드웨어에 크게 좌우된다"*를 본문·도표·경계 세 곳에 반복하고, **"측정 없이 고르지 않는다"**로 닫는다 |
| AOT Cache의 층위 | `07b` §5가 **"AOT Cache는 나머지 셋과 층이 다르다"** — 배타적 선택지가 아니라 표준 JVM 위에 얹는 개선이라는 구분을 명시한다. 표 하나에 나란히 놓여 생기는 오해를 미리 막는다 |
| CRaC의 통합 수준 | 배너가 `Started` 대신 `Restored`로 바뀐다는 구체적 근거로 *"언급만 되는 미래 기술이 아니라 이미 통합된 경로"*임을 보인다 |

**이 챕터가 정확한 이유는 인용이 많아서가 아니다**(노트당 0.42회). **불확실한 것을 불확실하다고 적었기 때문**이다 — 수치에 조건을 붙이고, 책과 실측이 다르면 둘 다 남기고, 층이 다른 것을 같은 표에 놓았을 때 그 사실을 경고한다. 오류가 난 챕터들과 갈리는 지점이 여기다.
