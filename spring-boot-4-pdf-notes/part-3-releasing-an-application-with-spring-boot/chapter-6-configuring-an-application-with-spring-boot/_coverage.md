...
> c4(자동 구성 내부)의 프로퍼티 조건, Ch1 `03b`(외부 설정)와 겹치는 영역이라 교차 대조했다.

### 결과 — 정정 0건

| 확인한 것 | 결과 |
|---|---|
| 프로퍼티 소스 우선순위 | `05` §2.1이 **15단계 전체**를 낮은→높은 순으로 제시하며, 테스트의 `@DynamicPropertySource`·`@TestPropertySource`와 DevTools 전역 설정까지 포함한다. 공식 순서와 일치 |
| `application.properties`의 위치 | 15개 중 **3번째로 낮다**는 사실과 그 함의("기준값을 정하기 좋은 자리")를 정확히 짚는다 |
| 프로파일은 대체가 아니라 가산 | `02` §2.3이 *"프로파일 파일은 기본 파일을 **대체하지 않는다**"*로 못박고, 프로파일에 없는 키가 살아남는 것을 도표로 보인다 |
| **리스트는 병합되지 않는다** | `02` §2.4가 *"프로파일 쪽 리스트가 **전체를 교체한다**"*, "기본 3개 + 프로파일 1개 = **최종 1개**"로 명시하고, **왜 그런 규칙인지**(병합 규칙을 하나로 정할 방법이 없다)까지 설명한다 |

**리스트 교체 규칙은 실무 사고가 잦은 지점**인데(기본 3명 + 테스트 1명 → 3명이 사라짐) 노트가 구체적 사고 시나리오까지 적어 두었다.
