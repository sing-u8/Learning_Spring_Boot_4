...
이 챕터는 저장소에서 **출처 표시가 0회**인 유일한 챕터라 우선 확인 대상이었다. 대조 결과 주장이 대부분 **구조적 사실**(JAR 레이아웃·레이어 메타데이터)이라 추론이 끼어들 여지가 적었다.

| 확인한 것 | 결과 |
|---|---|
| uber JAR의 구조 | `01`이 `BOOT-INF`·`classpath.idx`·`layers.idx`를 정확히 구분하고, `repackage`가 만드는 것과 표준 JAR 구조가 섞이지 않는 이유를 적는다 |
| `layers.idx`와 컨테이너 레이어의 연결 | `01` 7단계가 만든 것이 `02a`의 재료가 된다는 흐름이 정확하다 |
| 레이어 캐싱의 목적 | `02a` 제목 그대로 *"자바 파일 한 줄에 Spring을 다시 받지 않으려면"* — 변경 빈도가 다른 것을 다른 레이어에 두는 원리 |

**출처 표시 0회가 위험 신호는 아니었다.** 검증 가능한 사실이 구조적이라 도출 주장 자체가 적다. Ch4와 같은 유형이다.
