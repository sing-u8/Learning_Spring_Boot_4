...
source: "Spring Boot Reference — Data / SQL Databases (Open EntityManager in View) / Spring Framework — OpenEntityManagerInViewInterceptor / 대조 읽기: 김영한 『자바 ORM 표준 JPA 프로그래밍』 13장"
terms: [OSIV, 트랜잭션-경계]
...
- DB 커넥션이 요청이 끝날 때까지 반환되지 않는 것이 일반적이다. 언제든 지연 로딩이 일어날 수 있으니 붙잡고 있어야 하기 때문이다.

마지막이 가장 비싸다. 요청 처리에 200ms가 걸리는데 DB 작업은 20ms뿐이라면, 커넥션이 **필요 없는 180ms 동안에도 묶여 있다.** 커넥션 풀 크기가 곧 동시 처리 가능한 요청 수의 상한이 된다.
...
→ 비유가 깨지는 지점: 열람실을 열어 두는 비용은 좌석 하나다. OSIV의 비용은 그렇지 않다 — **DB 커넥션이라는 유한하고 공유되는 자원**을 붙잡는다. 좌석이 아니라 사서 한 명을 요청 내내 독점하는 쪽에 가깝고, 사서가 열 명뿐이면 열한 번째 요청은 대기한다. 이 자원의 성격 차이 때문에 "열어 두면 편하니까 열어 두자"가 성립하지 않는다.
...
5. **응답이 끝나면 인터셉터가 컨텍스트를 닫는다.** — 그때까지 커넥션도 함께 묶여 있다.
...
- 시작 경고가 사라진다.
- 트랜잭션이 끝나는 순간 컨텍스트가 닫히고 커넥션이 풀로 돌아간다.
...
[OSIV 켜짐 — 기본값]
  ...
  → 커넥션 풀이 10개면 초당 처리 가능 요청이 50건으로 묶인다
...
OSIV는 커넥션을 더 많이 쓰는 것이 아니라 **하나를 더 오래** 쓴다. 그래서 풀 크기를 늘려 대응하려는 발상이 나오는데, 근본 원인은 점유 시간이다. 시간을 줄이는 쪽이 맞는 방향이다.
...
- **템플릿 엔진을 쓰는 프로젝트라면 판단이 달라진다.** Thymeleaf 뷰에서 지연 로딩이 실제로 필요하다면 OSIV의 값이 있다. 그때도 커넥션 점유는 그대로이므로 풀 크기와 응답 시간을 함께 봐야 한다.
...
4. 커넥션 점유 시간이 처리량의 상한이 되는 과정을 숫자로 설명할 수 있는가?
