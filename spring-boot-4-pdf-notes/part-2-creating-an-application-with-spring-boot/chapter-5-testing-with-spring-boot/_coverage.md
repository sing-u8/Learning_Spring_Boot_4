...
테스트는 Boot 4에서 애노테이션이 여럿 바뀐 영역이라 **버전 민감 서술**을 집중 확인했다.

| 확인한 것 | 결과 |
|---|---|
| `@MockBean` → `@MockitoBean` | `03`이 **바뀐 이름을 쓰고**, 변경 사실·이유(*"Mockito를 쓴다는 사실을 명시하고 Spring 테스트 컨텍스트 안의 빈 오버라이드임을 분명히"*)·제약(**`@Configuration` 클래스에는 쓸 수 없다**)까지 적는다 |
| `@WebMvcTest` import 경로 변경 | `org.springframework.boot.webmvc.test.autoconfigure`로 정확히 기재하고, Ch15의 **모듈 세분화** 방향과 연결한다 |
| JUnit 6 기본 채택 | `01`이 책 Tip으로 출처를 밝히고, JUnit 5 테스트 호환·JUnit 4 레거시화까지 함께 적는다 |
| 테스트 컨텍스트 캐싱 | `01`이 Spring Test의 역할을 *"테스트용 애플리케이션 컨텍스트를 만들고 **캐시한다**"*로 명시한다 |

**버전 민감 서술에 전부 출처가 붙어 있다.** Ch11의 핀닝 오류(출처 없는 보강이 낡은 지식이었던 사례)와 대조된다.
