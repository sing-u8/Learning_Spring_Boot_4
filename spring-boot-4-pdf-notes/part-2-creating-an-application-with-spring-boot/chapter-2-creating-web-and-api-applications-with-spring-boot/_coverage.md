...
> 이 챕터는 `part-0-spring-core-internals` c3(MVC 요청 파이프라인)와 주제가 겹친다. c3는 Spring Framework 공식 문서를 1차 소스로 쓰고 대조 검증을 마쳤으므로, **두 트랙이 같은 것을 다르게 말하는 곳**을 찾는 방식으로 확인했다.

### 보강 1건

| # | 위치 | 문제 | 보강 |
|---|---|---|---|
| 1 | `05` §6 | *"`@RestController`가 이 클래스를 **JSON을 반환하는** 컨트롤러로 표시한다"* — 이 장의 맥락(클래스패스에 Jackson만 있음)에서는 결과가 맞지만, **`@RestController`가 형식을 정하는 것처럼 읽힌다** | `@RestController` = `@Controller` + `@ResponseBody`이며 **"본문에 직접 쓴다"는 뜻이지 "JSON으로 쓴다"는 뜻이 아니라는 것**, 형식은 **콘텐트 협상**의 결과라는 것, XML 컨버터가 딸려 오면 `Accept: */*` 클라이언트에게 같은 코드가 XML을 낼 수 있다는 것을 §6에 추가하고 c3로 연결 |

**모순이라기보다 정밀도 차이다.** Ch2는 입문 장이라 단순화가 정당하지만, 같은 저장소에 c3의 정밀한 서술이 생긴 이상 둘을 나란히 읽는 사람이 혼란을 겪는다. 한 줄 포인터로 그 간극을 메웠다.

### 정정 0건으로 확인한 것

| 확인한 것 | 결과 |
|---|---|
| `@Controller` vs `@RestController` 판별 | `02` §5의 판별 질문(*"메서드가 돌려준 문자열이 뷰 이름인가, 응답 본문인가?"*)이 c3 `03` §2.4의 "같은 `String`, 다른 운명"과 정확히 일치한다 |
| 논리적 뷰 이름 vs 파일 경로 | `02`가 *"파일 경로가 아니라 논리적 뷰 이름"*으로 구분하고 뷰 해석이 컨트롤러 바깥의 단계임을 명시한다. c3 `01`의 6단계 중 5단계와 일치 |
| "Jackson이 딸려 온다"의 근거 | *"마법이 아니라 전이 의존성 한 줄"*로 스타터 의존성 그래프까지 짚는다 |
| `05` §6의 경계 | record 필드 추가가 **공개 계약을 조용히 바꾼다**, POST는 멱등하지 않다, `@RequestBody`는 검증하지 않는다 — 전부 정확하고 실무적으로 중요한 지적이다 |
